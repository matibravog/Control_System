...
void loop() {
  ...
  if (myIMU.dataAvailable() == true) {
    ...
    // Raw data to m/s²
    float rawAx = myIMU.getRawAccelX() * (9.8 / 4096.0);
    float rawAy = myIMU.getRawAccelY() * (9.8 / 4096.0);
    float rawAz = myIMU.getRawAccelZ() * (9.8 / 4096.0);

    // Convert raw gyroscope data to rad/s
    float rawGx = myIMU.getRawGyroX() * (1 / 16.0) * (PI / 180.0);
    float rawGy = myIMU.getRawGyroY() * (1 / 16.0) * (PI / 180.0);
    float rawGz = myIMU.getRawGyroZ() * (1 / 16.0) * (PI / 180.0);

    float rawMx = myIMU.getRawMagX() * (1 / 16.0);
    float rawMy = myIMU.getRawMagY() * (1 / 16.0);
    float rawMz = myIMU.getRawMagZ() * (1 / 16.0);
    ...
    lowPassFilter(rawAx, ax, alpha);
    lowPassFilter(rawAy, ay, alpha);
    lowPassFilter(rawAz, az, alpha);

    lowPassFilter(rawGx, gx, alpha);
    lowPassFilter(rawGy, gy, alpha);
    lowPassFilter(rawGz, gz, alpha);

    lowPassFilter(rawMx, mx, alpha);
    lowPassFilter(rawMy, my, alpha);
    lowPassFilter(rawMz, mz, alpha);

    // Calculate pitch and roll using filtered accelerometer data
    float accelPitch = atan2(ay, sqrt(ax * ax + az * az)) * (180.0 / PI);
    float accelRoll = atan2(-ax, az) * (180.0 / PI);

    // Calculate yaw using filtered magnetometer data
    float magYaw = atan2(my, mx) * (180.0 / PI);

    // Update global matrices
    BLA::Matrix<3, 1> u = {gx, gy, gz}; // Use filtered gyroscope data
    BLA::Matrix<3, 1> Z = {accelPitch, accelRoll, magYaw}; // Use filtered accelerometer and magnetometer data
